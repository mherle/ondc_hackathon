...
function drawRoute(routingPath, selectedIndex){
    ...
    if(selectedIndex !== -1){
        ...
        selected_route_waypoints = routingPath[selected_route_path_index].map((item, index) => {
        ...
        function plotMarkers(){
            selected_route_waypoints.forEach((item, index) => {
                ...
                if (markerTitle){
                    (window.L.marker([item[0], item[1]], {icon: markerTitle === "Start" ? startIcon : endIcon}).addTo(map_init)).bindPopup("<b>" + markerTitle+ "</b><br></br>" + markerTitle + " path").openPopup();
                } else {
